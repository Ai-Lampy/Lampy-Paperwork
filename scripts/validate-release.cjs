...
const version=html.match(/const VERSION='([^']+)'/)[1];assert(html.includes('<title>Lampy Paperwork V'+version+'</title>'));assert(fs.readFileSync(path.join(root,'CHANGELOG.md'),'utf8').includes('V'+version));
