...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V46'));assert(JSON.stringify(doc).includes('Fixture'));assert(JSON.stringify(doc).length>300)}
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V46 browser release checks'));
