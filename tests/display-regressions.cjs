...
assert(source('renderPowerSheetView').includes('<th class=\"powerSocaColourCol\" scope=\"col\" aria-label=\"Colour\" title=\"Colour\"></th>'));
...
assert(source('footerTemplateControlMarkup').includes('Custom'));
...
assert(source('fitPowerPdfContent').includes('maxW/Math.max(1,content.scrollWidth)'));
assert(source('fitPowerPdfContent').includes('maxH/Math.max(1,content.scrollHeight)'));
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V47 browser release checks'));
console.log('PASS: V47 compound Power Fix IDs, constrained layout, per-distro PDF headers and final Totals page.');

// V47.1 temporary Power widths and compact Supply summary cards.
assert.equal(appVersion,'47.3');
...
assert(source('powerColumnWidthAdjusterMarkup').includes('data-power-width-field="min"'));
assert(source('powerColumnWidthAdjusterMarkup').includes('data-power-width-field="current"'));
assert(source('powerColumnWidthAdjusterMarkup').includes('data-power-width-field="max"'));
assert(source('resetPowerColumnWidths').includes('powerDefaultColumnWidths()'));
...
assert(compactSupply.includes('height:100px'));assert(compactSupply.includes('width:275px'));assert(compactSupply.includes('Distro A'));assert(!compactSupply.includes('Phase 1'));assert(!compactSupply.includes('Phase 2'));assert(!compactSupply.includes('Phase 3'));
...
assert(powerRenderSource.includes('class="powerSocaColourCol" scope="col" aria-label="Colour" title="Colour"></th>'));
assert(powerRenderSource.includes('class="wayCol" scope="col" aria-label="Way" title="Way"></th>'));
