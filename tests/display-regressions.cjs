...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V48'));assert(JSON.stringify(doc).toLowerCase().includes('browser'));assert(JSON.stringify(doc).length>300)}
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V48 browser release checks'));
...
// V47.11 displays three outlet colour fields and adds temporary per-table width controls.
assert.equal(appVersion,'48.2');
...
assert(source('powerExtraWidthAdjusterMarkup').includes("['min','current','max']"));
...
console.log('PASS: V47.11 Aux Way sizing, three inline colour fields and session-only width adjusters.');

// V47.12 adopts the reviewed Aux and 3ø Min, Current and Max width profiles.
assert.equal(appVersion,'48.2');
const v4712WidthDefaults=html.slice(html.indexOf('const POWER_EXTRA_COLUMN_WIDTH_DEFAULTS='),html.indexOf('let powerExtraWidthSettings=',html.indexOf('const POWER_EXTRA_COLUMN_WIDTH_DEFAULTS=')));
for(const expected of ["way:{label:'Way',min:25,current:25,max:25","label:{label:'Label',min:50,current:125,max:200","watts:{label:'Watts',min:40,current:70,max:75","amps:{label:'Amps',min:50,current:70,max:80","include:{label:'Include',min:50,current:70,max:80","colours:{label:'Colours',min:100,current:200,max:200","type:{label:'Type',min:50,current:70,max:80","label:{label:'Label',min:50,current:150,max:200"])assert(v4712WidthDefaults.includes(expected),expected);
...
assert(html.includes('width:var(--power-aux-watts-width,70px)!important'));
assert(html.includes('width:var(--power-aux-amps-width,70px)!important'));
assert(html.includes('width:var(--power-aux-include-width,70px)!important'));
...
console.log('PASS: V47.12 reviewed Aux and 3ø width defaults and Reset profiles.');
