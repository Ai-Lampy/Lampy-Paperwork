...
// V48.1 simplifies phase cards and links each Supply to its assigned Distros.
assert.equal(appVersion,'48.1');
const v481PhaseMarkup=source('phaseTotalsMarkup');
const v481SupplyMarkup=source('powerSupplyCardMarkup');
assert(!v481PhaseMarkup.includes('Distro Phase Totals'));
assert(!v481SupplyMarkup.includes('supplyDistroNames'));
assert.equal((v481SupplyMarkup.match(/powerSupplyMeta/g)||[]).length,1);
assert(v481SupplyMarkup.includes("supply.input||'Input Supply not selected'"));
...
const v481Context=vm.createContext({Math,Number,parseInt,Array});
vm.runInContext(source('powerPhaseConnectorGraphic'),v481Context);
vm.runInContext(source('powerPhaseLinkedGroupMarkup'),v481Context);
const singleLink=v481Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',['<div>Distro A</div>']);
const multiLink=v481Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',['<div>Distro A</div>','<div>Distro B</div>']);
assert(singleLink.includes('data-power-phase-links="1"'));
assert(singleLink.includes('aria-hidden="true"'));
assert(!singleLink.includes('<line x1="36" y1="50" x2="36"'));
assert(multiLink.includes('data-power-phase-links="2"'));
assert(multiLink.includes('<line x1="36" y1="50" x2="36" y2="150"'));
assert.equal((multiLink.match(/<polygon/g)||[]).length,2);
assert.equal(v481Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',[]),'<div>Supply</div>');
...
console.log('PASS: V48.1 simplified Project Font phase cards and live Supply-to-Distro connectors.');
