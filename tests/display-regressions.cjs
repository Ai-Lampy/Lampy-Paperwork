...
assert.equal(appVersion,'40.5');
assert(html.includes('.loomsToolbar{display:flex;justify-content:space-between;gap:12px;align-items:center;border:2px solid #000;border-radius:8px;background:rgb(185,185,185);padding:2px 12px}'));
assert(html.includes('.loomCardHeading{cursor:pointer;position:relative;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;min-height:60px;padding:0 12px'));
...
assert(source('loomCardMarkup').includes('Add Extension')&&source('loomCardMarkup').includes('loomCardSubheading'));
...
assert.equal(appVersion,'40.5');
assert(html.includes('.loomCardSubheading{margin-top:2px;text-align:center;font-family:var(--top-font);font-size:16px;font-weight:900}'));
assert(html.includes('.loomCardExtensionButton{grid-column:1;justify-self:start;transform:scale(.8);transform-origin:left center'));
