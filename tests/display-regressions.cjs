...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V42'));assert(JSON.stringify(doc).includes('Fixture'));assert(JSON.stringify(doc).length>300)}
...
for(const name of ['normaliseImportMatch','fixtureOverrideKey','normaliseFixtureOverrides','normaliseLabelPreview','defaultProjectInfo','normaliseProjectInfo'])vm.runInContext(source(name),pctx);
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V42 browser release checks'));
...
assert(source('renderHomeView').includes('fixtureBody=fixtureSummaryMarkup()'));
