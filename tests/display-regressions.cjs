...
assert(source('loomCardMarkup').includes('Edit Loom')&&source('loomCardMarkup').includes('Delete Loom'));
...
assert.equal(appVersion,'40.4');
assert(html.includes('.loomCardActions{grid-column:3;justify-self:end;display:flex;flex-direction:column;gap:3px;transform:scale(.8);transform-origin:right center}'));
...
assert.equal(appVersion,'40.4');
assert(html.includes('.loomsToolbar{display:flex;justify-content:space-between;gap:12px;align-items:center;border:2px solid #000;border-radius:8px;background:rgb(185,185,185);padding:2px 12px}'));
assert(html.includes('.loomCardHeading{cursor:pointer;position:relative;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;min-height:68px;padding:0 12px'));
