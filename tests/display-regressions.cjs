...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V44'));assert(JSON.stringify(doc).includes('Fixture'));assert(JSON.stringify(doc).length>300)}
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V44 browser release checks'));
...
console.log('PASS: V44 Project Settings navigation, saved defaults, typography, units, voltage, logos and PDF chrome.');
