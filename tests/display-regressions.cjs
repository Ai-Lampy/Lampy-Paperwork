...
assert(html.includes('<title>Lampy Paperwork V33.8</title>'));
...
for(const name of ['normalisePatchMode','normaliseFixtureGdtf','normaliseFixtureModeAliases','fixtureCanonicalMode','normaliseFixtureList'])vm.runInContext(source(name),modeContext);
...
for(const name of ['fixtureCanonicalMode','libraryFixtureForPatchRow','migratePatchModeNames'])vm.runInContext(source(name),migrationContext);
...
assert(fs.existsSync(root+'MODE_NAME_REVIEW.csv'));
