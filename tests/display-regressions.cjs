...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V41'));assert(JSON.stringify(doc).includes('Fixture'));assert(JSON.stringify(doc).length>300)}
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V41 browser release checks'));
...
assert(html.includes('-webkit-text-stroke-width:.8mm;-webkit-text-stroke-color:#000;paint-order:stroke fill'));
const loomStyleContext=vm.createContext({Array});vm.runInContext(source('loomHeadingBackground'),loomStyleContext);
assert.equal(loomStyleContext.loomHeadingBackground(['#00b311']),'#00b311');
assert(loomStyleContext.loomHeadingBackground(['#00b311','#ff0000']).includes('repeating-linear-gradient(135deg, #00b311 -2mm 7mm, #ff0000 7mm 16mm)'));
assert(loomStyleContext.loomHeadingBackground(['#00b311','#ff0000','#0060d2']).includes('#0060d2 10mm 16mm'));
...
assert.equal(appVersion,'41');
assert(html.includes('.loomCardActions{grid-column:3;justify-self:end;display:flex;flex-direction:row;gap:4px;align-items:center;transform:scale(.8);transform-origin:right center}'));
...
assert.equal(appVersion,'41');
assert(html.includes('.loomsToolbar{display:flex;justify-content:space-between;gap:12px;align-items:center;border:2px solid #000;border-radius:8px;background:rgb(185,185,185);padding:2px 12px}'));
assert(html.includes('.loomCardHeading{cursor:pointer;position:relative;display:grid;grid-template-columns:1fr minmax(0,auto) 1fr;align-items:center;gap:10px;height:40px;min-height:40px;max-height:55px;padding:0 12px'));
...
assert.equal(appVersion,'41');
assert(html.includes('.loomCardSubheading{grid-column:1;justify-self:start;margin:0;padding:0 6px;text-align:left;font-family:var(--top-font);font-size:18px;font-weight:900;color:#000;background:#fefefe;border:1px solid #000;border-radius:30px}'));
assert(html.includes('.loomCardExtensionButton{padding:4px 6px;font-size:18px;font-weight:800}'));
...
assert(html.includes('.loomCardHeading{cursor:pointer;position:relative;display:grid;grid-template-columns:1fr minmax(0,auto) 1fr;align-items:center;gap:10px;height:40px;min-height:40px;max-height:55px;padding:0 12px;background:var(--loom-background,#fff)'));
...
assert(source('normaliseHomeLayout').includes("storedId==='socapexes'?'looms':storedId"));
...
assert.equal(migratedLoomLayout.stats[0].id,'looms');assert.equal(migratedLoomLayout.stats[0].size,'wide');assert.equal(migratedLoomLayout.stats[0].visible,false);
