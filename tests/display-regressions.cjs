...
add('escapeJsAttr','safeEncodeURIComponent','deviceConfigSortHeader','deviceConfigTableSortState','deviceConfigHeaderKey');
vm.runInContext('const deviceConfigTableSorts=new Map();',c);
...
c.deviceConfigExpandedDevices=new Set();c.controlExpandedDevices=new Set();c.render=()=>{};c.ipDeviceOrderKey=d=>d.source+':'+d.id;c.compiledDeviceConfigDevices=()=>[{source:'console',id:'a'},{source:'npu',id:'p'}];c.activeControlNetworkTab='control';
...
c.expandAllControlDevices();assert(c.controlExpandedDevices.has('npu:p'));assert(c.controlExpandedDevices.has('console:a'));c.collapseAllControlDevices();assert.equal(c.controlExpandedDevices.size,0);assert(c.deviceConfigExpandedDevices.has('console:a'));
c.collapseAllDeviceConfig();assert.equal(c.deviceConfigExpandedDevices.size,0);assert.equal(c.controlExpandedDevices.size,0);
for(const name of ['clearProjectState','loadProjectPayload']){const body=source(name);assert(body.includes('controlExpandedDevices.clear()'));assert(body.includes('deviceConfigExpandedDevices.clear()'));assert(body.includes('controlOpenLocations.clear()'));assert(body.includes("controlTableSort={key:'name',direction:'asc'}"))}
...
c.deviceConfigExpandedDevices.clear();c.controlExpandedDevices.clear();c.activeControlNetworkTab='control';c.controlExpandedDevices.add('console:a');
...
assert(control.includes("'console:a','control'"));assert(config.includes("'console:a','deviceConfig'"));assert(control.includes('>Capacity</th>'));assert(control.includes('32 Uni'));assert(!control.includes('System Limit:'));
...
assert(source('drawPositionSummaryBlock').includes('colourSetPresentation(colours)'));
...
assert(html.includes(`<title>Lampy Paperwork V${appVersion}</title>`));
assert(html.includes(".powerSocaNameText{position:absolute;inset:3px 6px;display:flex;align-items:center;justify-content:center;overflow:hidden;white-space:normal;overflow-wrap:normal;word-break:normal;text-align:center;line-height:1;cursor:text;-webkit-text-stroke-width:.5mm}"));
...
assert(source('rearSocaBackground').includes('rearLabelBackground(rearSocaColours(meta))'));
assert(source('makeRearLabel').includes('rearSocaBackground(meta)'));
assert(source('updateRearSocaLabel').includes('rearLabelBackground(colours)'));
...
assert(html.includes('.fixturePatchColour{font-weight:900;text-transform:uppercase;border:1px solid #000;-webkit-text-stroke-width:var(--colour-text-stroke-width,0)'));
...
assert(source('drawPdfRearLabelStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,30,.5)'));
assert(source('preparePdfSocaColourLayers').includes("[data-rear-label-colours]"));
...
assert(source('updateRearSocaLabel').includes('rearLabelColours=colours.join'));
assert(source('cssStripe').includes("positionColourBackground(colours,w,'mm','.3mm')"));
assert(source('drawPdfRearLabelStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,30,.5)'));
const v358=vm.createContext({$:()=>({value:'4'}),escapeAttr:value=>String(value),normaliseHex:value=>String(value||'').toLowerCase(),colourTextToHex:value=>String(value||'')});
for(const name of ['rearSocaColours','outlinedStripeBandStops','positionStripeBands','positionColourBackground','rearLabelBackground','rearSocaBackground','cssStripe'])vm.runInContext(source(name),v358);
...
assert(source('controlPositionStripStyle').includes('colourSetPresentation(colours)'));
assert(source('controlLocationCellStyle').includes('powerPositionCellStyle({positionColours:colours})'));
...
assert(html.includes('.controlPositionName{-webkit-text-stroke-width:.5mm;-webkit-text-stroke-color:var(--pos-text-outline,transparent);paint-order:stroke fill;text-shadow:none!important}'));
...
assert(source('applyBackgroundColourTextPresentation').includes("outlineWidth='.5mm'"));
...
assert(source('powerSheetRowMarkup').includes('--power-soca-outline:${socaPresentation.outline}'));
...
assert.equal(v406.colourSetPresentation(['#FFFFFF']).text,'#111111');
assert.equal(v406.colourSetPresentation(['#FFFFFF']).outline,'transparent');
assert(v406.loomHeadingStyle({colour1:'#FFFF3D'}).includes('--loom-text:#ffffff'));
assert(v406.loomHeadingStyle({colour1:'#ffffff'}).includes('--loom-title-stroke-width:0'));
...
assert(source('drawPositionSummaryBlock').includes('colourSetPresentation(colours)'));
assert(source('controlPositionStripStyle').includes("positionSummary?'.5mm':'.3mm'"));
assert(html.includes('.loomCardHeading{cursor:pointer;position:relative;display:grid;grid-template-columns:minmax(0,160px) minmax(0,1fr) minmax(0,160px);align-items:center;gap:4px;height:40px;min-height:40px;max-height:55px;padding:0 4px'));
assert(html.includes('.loomCardSubheading{grid-column:1;align-self:center;justify-self:start;min-width:0;max-width:100%;flex:0 0 auto;white-space:nowrap;margin:0;padding:0 6px;text-align:left;font-family:var(--project-subheader-font);font-size:18px;font-weight:900;color:#000;background:#fefefe;border:1px solid #000;border-radius:30px}'));
assert(html.includes('.loomEndsTogetherRow td{color:#c00000;font-weight:900;border-top:2px solid #000!important}'));
...
assert(source('controlPositionStripStyle').includes("positionStripeBands(colours,30,'px',positionSummary?'.5mm':'.3mm')"));
...
assert(source('powerSheetRowMarkup').includes('powerSocaMultiColourBackground(socaColourValues)'));
assert(source('fanOutRowMarkup').includes('powerSocaMultiColourBackground(socaColourValues)'));
assert(source('fanOutSlotCellsMarkup').includes('powerPositionCellStyle(result)'));
assert(source('drawPdfSocaStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,stripeMm*96/25.4,.3)'));
...
const v435LoomContext=vm.createContext({colourTextToHex:value=>String(value||''),normaliseHex:value=>String(value||''),colourSetPresentation:values=>({text:values.length===1&&String(values[0]).toLowerCase()==='#ffffff'?'#111111':'#ffffff',outline:values.length===1&&String(values[0]).toLowerCase()==='#ffffff'?'transparent':'#000000'}),loomHeadingBackground:()=>''});
...
assert(source('createDistroDraft').includes("String(projectDefaultVoltage())+'V'"));
...
assert(html.includes('a.reportIssueBtn,.homeHero,.homeHero *,.homeGrid,.homeGrid *,.consoleControlCard,.consoleControlCard *,.npuControlCard,.npuControlCard *,.rackDeviceLibraryCard,.rackDeviceLibraryCard *,.rackSettingsCard,.rackSettingsCard *{font-family:var(--app-font)}'));
...
const supplyCardContext=vm.createContext({escapeAttr:value=>String(value),escapeHtml:value=>String(value),powerSupplyTotals:()=>({p1:1,p2:2,p3:3}),formatPhaseAmps:value=>value+' A'});
...
assert(source('powerPdfDistroSummaryMarkup').includes('powerPdfSupplyCardMarkup'));
assert(source('powerPdfDistroSummaryMarkup').includes('phaseTotalsMarkup'));
...
assert(source('fitPowerPdfContent').includes('layout.tablePage&&!layout.mainPowerTable?layout.widthScale'));
assert(source('powerPdfContentWouldOverflow').includes('layout.tablePage&&!layout.mainPowerTable?layout.widthScale'));
...
assert(!html.includes('.exportPage.pdfLaterPage{padding-top:14mm}'));
assert(source('hideEmptyPowerPdfColumns').includes("column.style.setProperty('display','none','important')"));
assert(source('expandSparsePowerPdfColumns').includes("if(content.querySelector('.fanOutView'))return false"));
assert(source('powerPdfExpandedColumnWidths').includes("['fixId','fixType','position'].includes"));
assert(source('powerPdfContentLayout').includes("page.querySelector('.pdfPageFooter')"));
assert(source('powerPdfContentLayout').includes('content.offsetTop'));
assert(source('powerPdfContentLayout').includes('footer.offsetTop'));
assert(source('powerPdfContentLayout').includes('footerTop-8'));
const v478WidthContext=vm.createContext({});
vm.runInContext(source('powerPdfExpandedColumnWidths'),v478WidthContext);
for(const groups of [1,2,3]){
  const columns=[{key:'socapex',width:112},{key:'way',width:26}];
  for(let index=0;index<groups;index++)columns.push({key:'fixId',width:59},{key:'fixType',width:117},{key:'position',width:115});
  columns.push({key:'watts',width:45},{key:'amps',width:72});
  const target=columns.reduce((sum,column)=>sum+column.width,0)+300,widths=Array.from(v478WidthContext.powerPdfExpandedColumnWidths(columns,target));
  assert(Math.abs(widths.reduce((sum,width)=>sum+width,0)-target)<.001);
  assert.equal(widths[0],112);assert.equal(widths[1],26);assert.equal(widths.at(-2),45);assert.equal(widths.at(-1),72);
  assert(Math.abs((widths[2]/widths[3])-(59/117))<.0001);
  assert(Math.abs((widths[3]/widths[4])-(117/115))<.0001);
}
...
assert(source('addDistroCard').includes('distroDraft=createDistroDraft()'));
assert(!source('addDistroCard').includes('app.distros.push'));
assert(source('saveDistroDraft').includes('app.distros.push(saved)'));
assert(source('savePowerSupplyDraft').includes('info.powerSupplies.push(draft)'));
assert(source('closeDistroSettings').includes("distroDraft=null"));
assert(source('closePowerSupplyPane').includes("powerSupplyDraft=null"));
assert(source('powerAuxSheetMarkup').includes('aria-label="Way"'));
assert(source('powerSheetAuxRowMarkup').includes('powerExtraLabelCellMarkup'));
assert(source('powerOutputSheetMarkup').includes('<th class="powerOutputTypeCol">Type</th><th class="auxLabelCol">Label</th>'));
assert(!source('powerOutputSheetMarkup').includes('Output ${index+1}'));
assert(source('powerExtraLabelColours').includes('useC2'));
assert(source('powerExtraLabelColours').includes('useC3'));
assert(source('powerExtraLabelCellMarkup').includes('data-power-position-colours'));
assert(source('renderPowerPdfPreview').includes("'extras'"));
assert(!source('renderPowerPdfPreview').includes("'aux'"));
assert(!source('renderPowerPdfPreview').includes("'output'"));
assert(source('preparePowerPdfView').includes("rowMode==='extras'"));
...
assert(source('powerSheetAuxRowMarkup').indexOf('auxIncludeCol')<source('powerSheetAuxRowMarkup').indexOf('powerExtraColourCellMarkup'));
assert(source('powerOutputSheetMarkup').indexOf('powerExtraLabelCellMarkup')<source('powerOutputSheetMarkup').indexOf('powerExtraColourCellMarkup'));
assert(source('powerAuxSheetMarkup').includes('<th class="powerExtraColourCol">Colours</th>'));
assert(source('powerOutputSheetMarkup').includes('<th class="powerExtraColourCol">Colours</th>'));
...
assert.equal((source('powerExtraColourCellMarkup').match(/field\('c[123]'\)/g)||[]).length,3);
assert(!html.includes('powerExtraWidthAdjuster'));
assert(!html.includes('powerExtraWidthSettings'));
...
assert(html.includes('-webkit-text-stroke-width:var(--power-extra-stroke,0)'));
assert(source('powerExtraLabelCellStyle').includes("outline==='transparent'?'0':'1mm'"));
const v4713FitSource=source('fitPowerExtraLabelText');
assert(v4713FitSource.includes('minimum=12,maximum=30'));
assert(v4713FitSource.includes('element.clientWidth-horizontalPadding-(stroke*2)'));
assert(v4713FitSource.includes('element.clientHeight-verticalPadding-(stroke*2)'));
assert(v4713FitSource.includes('powerSheetTextWidth(text,style,size)'));
assert(source('renderPowerSheetView').includes('fitPowerExtraLabelText(input)'));
...
assert(html.includes('.powerAuxSheet tbody .auxWattsCol,.powerAuxSheet tbody .auxWattsCol input,.powerAuxSheet tbody .ampsCol,.powerAuxSheet tbody .auxIncludeCol,.powerAuxSheet tbody .powerExtraColourCol{padding:3px!important}'));
assert(html.includes('.powerExtraColourFields{display:grid;grid-template-columns:repeat(3,70px)'));
...
v4714Context.retainPowerAuxColourSettings({d:{}},0,{querySelectorAll:()=>[
  {dataset:{powerExtraColour:'c1'},value:'Red'},
  {dataset:{powerExtraColour:'c2'},value:'Blue'},
  {dataset:{powerExtraColour:'c3'},value:''}
]});
assert.deepEqual(v4714Label,{c1:'#ff0000',c2:'#0000ff',c3:'',useC2:true,useC3:false});
assert(source('togglePowerAuxInclude').includes("retainPowerAuxColourSettings(range,index,button.closest('tr'))"));
assert(!source('togglePowerAuxInclude').includes('powerExtraWidthSettings='));
...
assert(html.includes('.previewPanel{padding:var(--app-inner-gutter);border:0'));
assert.equal(1500-(2*25),1450);
assert.equal(1450-(2*20),1410);
assert(html.includes('.liveResponsiveScaleContent{position:absolute;top:0;left:0;min-width:0;transform-origin:top left'));
assert(html.includes('.liveResponsiveScaleHost{position:relative;width:100%;max-width:100%;min-width:0;overflow:hidden'));
assert(html.includes('.liveResponsiveScaleHost{height:auto!important;overflow:visible!important}'));
assert(html.includes('.liveResponsiveScaleContent{position:static!important;width:auto!important;transform:none!important}'));
const v48Targets=html.slice(html.indexOf('const LIVE_RESPONSIVE_TARGETS='),html.indexOf('function liveResponsiveScaleValue(',html.indexOf('const LIVE_RESPONSIVE_TARGETS=')));
for(const expected of [
  "kind:'fixture',natural:1400,fill:true",
  "kind:'power',natural:1400,fill:true",
  "kind:'fanOut',natural:1250,fill:true",
  "kind:'rack',natural:0,fill:true",
  "kind:'labels',natural:1300,fill:true",
  "kind:'device',natural:1220,fill:false",
  "kind:'ip',natural:1250,fill:true",
  "kind:'tableView',natural:0,fill:false"
])assert(v48Targets.includes(expected),expected);
const v48Context=vm.createContext({Math,Number});
vm.runInContext(source('liveResponsiveScaleValue'),v48Context);
assert.equal(v48Context.liveResponsiveScaleValue(1410,1400,true),1410/1400);
assert.equal(v48Context.liveResponsiveScaleValue(700,1400,true),0.5);
assert.equal(v48Context.liveResponsiveScaleValue(1410,1220,false),1);
assert.equal(v48Context.liveResponsiveScaleValue(610,1220,false),0.5);
assert(source('observeResponsiveSheetElement').includes("window.addEventListener('resize',scheduleResponsiveSheetLayout)"));
assert(source('observeResponsiveSheetElement').includes("document.fonts?.addEventListener('loadingdone',scheduleResponsiveSheetLayout)"));
assert(source('observeResponsiveSheetElement').includes('new MutationObserver'));
assert(source('fitLiveResponsiveHost').includes("host.style.height=height+'px'"));
assert(source('render').includes("observeResponsiveSheetElement(sheet.closest('.sheetWrap')||sheet)"));
...
assert(!v482SupplyMarkup.includes('supplyDistroNames'));
assert.equal((v482SupplyMarkup.match(/powerSupplyMeta/g)||[]).length,1);
assert(v482SupplyMarkup.includes("supply.input||'Input Supply not selected'"));
...
assert(!html.includes('.homeGrid *,.powerSupplyCard,.powerSupplyCard *,.consoleControlCard'));
...
const v482Context=vm.createContext({Math,Number,parseInt,Array});
vm.runInContext(source('powerPhaseConnectorGraphic'),v482Context);
vm.runInContext(source('powerPhaseLinkedGroupMarkup'),v482Context);
vm.runInContext(source('powerPhaseConnectorGeometry'),v482Context);
const singleLink=v482Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',['<div>Distro A</div>']);
const multiLink=v482Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',['<div>Distro A</div>','<div>Distro B</div>']);
assert(singleLink.includes('data-power-phase-links="1"'));
assert(singleLink.includes('aria-hidden="true"'));
assert(singleLink.includes('data-power-phase-trunk'));
assert(singleLink.includes('hidden'));
assert(multiLink.includes('data-power-phase-links="2"'));
assert(multiLink.includes('data-power-phase-branch="0"'));
assert(multiLink.includes('data-power-phase-branch="1"'));
assert.equal((multiLink.match(/<polygon/g)||[]).length,2);
assert.equal(v482Context.powerPhaseLinkedGroupMarkup('<div>Supply</div>',[]),'<div>Supply</div>');
const geometry=JSON.parse(JSON.stringify(v482Context.powerPhaseConnectorGeometry(
  {top:100,height:210},
  {top:160,height:90},
  [{top:110,height:60},{top:230,height:70}]
)));
assert.deepEqual(geometry,{height:210,supplyY:105,branchYs:[40,165],trunkStart:40,trunkEnd:165});
...
assert(source('fitPowerPhaseLinkedGroup').includes("arrow.setAttribute('points'"));
...
assert(source('powerSuppliesMarkup').includes('powerPhaseLinkedGroupMarkup(powerSupplyCardMarkup(supply,ranges),distroCards)'));
assert(source('powerPhaseTotalsOverviewMarkup').includes('powerPhaseLinkedGroupMarkup(powerSupplyCardMarkup(supply,ranges),distroCards)'));
...
assert(source('preparePowerPdfTotals').includes('powerPhaseLinkedGroupMarkup'));
const v484SummaryContext=vm.createContext({
  powerPdfAssignedSupply:range=>range.supply||null,
  powerPdfSupplyCardMarkup:()=>'<div class="supply">Supply</div>',
  phaseTotalsMarkup:name=>`<div class="distro">${name}</div>`,
  powerSheetPhaseTotals:()=>({}),
  powerPhaseLinkedGroupMarkup:(supply,cards)=>`<div class="linked" data-count="${cards.length}">${supply}${cards.join('')}</div>`
});
vm.runInContext(source('powerPdfDistroSummaryMarkup'),v484SummaryContext);
const v484Assigned=v484SummaryContext.powerPdfDistroSummaryMarkup({idx:0,d:{name:'Distro A'},supply:{}},[]);
const v484Unassigned=v484SummaryContext.powerPdfDistroSummaryMarkup({idx:1,d:{name:'Distro B'}},[]);
assert(v484Assigned.includes('class="linked" data-count="1"'));
assert(v484Assigned.includes('class="supply"'));
assert(v484Assigned.includes('class="distro"'));
assert(!v484Unassigned.includes('class="linked"'));
assert(v484Unassigned.includes('class="distro"'));
...
assert(source('activeApplicationPageName').includes("return 'Control'"));
const unifiedControlSource=source('renderConsolesTab');
assert(unifiedControlSource.includes("controlUnifiedSectionMarkup('console')"));
assert(unifiedControlSource.includes("controlUnifiedSectionMarkup('npu')"));
assert(unifiedControlSource.indexOf("controlUnifiedSectionMarkup('console')")<unifiedControlSource.indexOf("controlUnifiedSectionMarkup('npu')"));
assert(source('controlCardsMarkup').includes("No ${type==='npu'?'NPUs':'consoles'} added yet."));
assert(source('controlTableGroupsMarkup').includes('controlEmptyState'));
assert(source('controlTableGroupsMarkup').includes('deviceConfigLocationGroup controlLocationGroup'));
assert(source('controlTableGroupsMarkup').includes('setControlLocationOpen'));
assert(source('expandAllControlDevices').includes("device.source==='console'||device.source==='npu'"));
assert(source('collapseAllControlDevices').includes("device.source==='console'||device.source==='npu'"));
assert(html.includes('.controlNetworkToolbar .powerSheetToolbarActions .btn{width:auto;min-width:0;padding:8px;font-family:var(--app-font);font-size:14px;font-weight:800}'));
assert(html.includes('.controlNetworkView .controlPositionName{font-family:var(--project-body-font)}'));
assert(source('controlDeviceConfigTableMarkup').includes('data-device-config-width-table="true"'));
assert(source('controlDeviceConfigTableMarkup').includes('setControlTableSort'));
assert(source('controlSortDevices').includes("controlTableSort.direction==='desc'?-1:1"));
assert(source('clearProjectState').includes('controlOpenLocations.clear()'));
assert(source('loadProjectPayload').includes('controlKnownLocations.clear()'));
assert(source('controlLocationCellStyle').includes('powerPositionCellStyle({positionColours:colours})'));
assert(source('controlLocationCellStyle').includes('colourSetPresentation(colours)'));
assert(source('controlLocationCellStyle').includes('--control-location-text:${presentation.text}'));
assert(source('controlLocationCellStyle').includes('powerPositionCellStyle({positionColours:colours})'));
assert(source('powerMultiColourBackground').includes("positionStripeBands(values,stripeWidth,'mm','.3mm')"));
assert(html.includes('-webkit-text-stroke-width:var(--power-position-stroke-width,0)!important'));
...
const v492Colour=vm.createContext({
  ...
  colourTextToHex:(value,fallback='')=>String(value||fallback)
});
for(const name of ['colourSetPresentation','outlinedStripeBandStops','positionStripeBands','positionColourBackground','rearLabelBackground'])vm.runInContext(source(name),v492Colour);
assert.deepEqual(JSON.parse(JSON.stringify(v492Colour.colourSetPresentation([]))),{colours:[],text:'#111111',outline:'transparent',shadow:'none'});
assert.equal(v492Colour.colourSetPresentation(['#ffffff']).text,'#111111');
assert.equal(v492Colour.colourSetPresentation(['#ffffff']).outline,'transparent');
for(const colours of [['#ff0000'],['#000000'],['#ffff3d','#ffffff'],['#ff0000','#ffffff','#0066cc']]){
  const presentation=v492Colour.colourSetPresentation(colours);
  assert.equal(presentation.text,'#ffffff');
  assert.equal(presentation.outline,'#000000');
}
assert.equal(v492Colour.rearLabelBackground(['#ffff3d','#ffffff']),'repeating-linear-gradient(135deg,#ffff3d 0px,#ffff3d 30px,#000 30px,#000 calc(30px + .5mm),#ffffff calc(30px + .5mm),#ffffff calc(60px - .5mm),#000 calc(60px - .5mm),#000 60px)');
assert.equal(v492Colour.rearLabelBackground(['#ff0000','#ffffff','#0066cc']),'repeating-linear-gradient(135deg,#ff0000 0px,#ff0000 30px,#000 30px,#000 calc(30px + .5mm),#ffffff calc(30px + .5mm),#ffffff calc(60px - .5mm),#000 calc(60px - .5mm),#000 60px,#0066cc 60px,#0066cc 90px)');
assert(source('controlPositionStripStyle').includes("positionSummary?'.5mm':'.3mm'"));
assert(source('powerMultiColourBackground').includes("positionStripeBands(values,stripeWidth,'mm','.3mm')"));
assert(source('cssStripe').includes("positionColourBackground(colours,w,'mm','.3mm')"));
assert(source('loomHeadingBackground').includes("outlinedStripeBandStops(values[1],4,10,'mm')"));
assert(source('rearLabelBackground').includes("positionColourBackground(colours,30,'px','.5mm')"));
assert(html.includes('.homePositionStrip .controlPositionName,.positionPdfItem span{-webkit-text-stroke-width:1.3mm'));
assert(html.includes('.controlPositionName{-webkit-text-stroke-width:.5mm'));
assert(html.includes('.controlNetworkView .consolePositionName{-webkit-text-stroke-width:.5mm'));
assert(html.includes('.deviceConfigPortLocationCell{background-color:var(--pos-base,#fff);background-image:var(--pos-pattern,none);color:var(--pos-text,#111);text-shadow:none;-webkit-text-stroke-width:.5mm'));
assert(html.includes('.fixturePatchColour{font-weight:900;text-transform:uppercase;border:1px solid #000;-webkit-text-stroke-width:var(--colour-text-stroke-width,0)'));
assert(html.includes('.patchPdfColourPill{display:flex;align-items:center;justify-content:center;min-height:7mm')&&html.includes('-webkit-text-stroke-width:var(--colour-text-stroke-width,0)'));
assert(source('powerExtraLabelCellStyle').includes("presentation.outline==='transparent'?'0':'1mm'"));
assert(source('loomHeadingStyle').includes("presentation.outline==='transparent'?'0':'.8mm'"));
assert(source('makeRearLabel').includes('rearSocaBackground(meta)'));
for(const name of ['makeRearAuxLabel','makeRearOutputLabel','updateRearSocaLabel','updateRearAuxLabel','updateRearOutputLabel'])assert(source(name).includes('rearLabelBackground'));
assert(source('preparePdfSocaColourLayers').includes('[data-front-label-colours]:not(.spareAuto)'));
assert(source('preparePdfSocaColourLayers').includes('[data-rear-label-colours]:not(.spareAuto)'));
assert(source('preparePdfSocaColourLayers').includes('drawPdfRearLabelStripeCanvas'));
assert(source('drawPdfRearLabelStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,30,.5)'));
assert(source('drawPdfSocaStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,stripeMm*96/25.4,.3)'));
assert(source('pdfDrawPositionText').includes('1.3*PDF_MM_TO_PT'));
assert(source('pdfDrawOutlinedPillText').includes('.5*PDF_MM_TO_PT'));
assert(source('pdfDrawOutlinedCellText').includes('strokeWidthMm*PDF_MM_TO_PT'));
assert(source('powerPdfDomPdfBytes').includes("strokeWidth=cell.classList.contains('powerExtraLabelCell')?1:.5"));
assert(source('powerPdfDomPdfBytes').includes("!cell.classList.contains('spareAuto')"));
assert(html.includes(':root{--label-w:18mm;--label-h:34mm'));
assert(html.includes('.label{width:var(--label-w);height:var(--label-h);border-radius:var(--radius);border:var(--border) solid var(--border-col);overflow:hidden;position:relative;background:#fff;color:#000;display:grid;grid-template-rows:1fr 1fr'));
assert(html.includes('.label .topHalf{font-size:var(--top-size);border-bottom:.35mm solid rgba(0,0,0,.6)'));
assert(html.includes('.gridLine{outline:.25mm solid #278aff;outline-offset:-.25mm}'));
...
assert(!html.includes('function patchColourTextShadow('));
...
assert(source('colourInputStyle').includes("--colour-text-outline:${style.outline}"));
assert(source('colourInputStyle').includes("style.outline==='transparent'?'0':'.5mm'"));
assert(source('applyColourFieldPresentation').includes("style.setProperty('--colour-text-stroke-width',style.outline==='transparent'?'0':'.5mm')"));
const v493Field=vm.createContext({colourTextToHex:value=>String(value||''),normaliseHex:value=>String(value||'').toLowerCase()});
vm.runInContext(source('colourSetPresentation'),v493Field);vm.runInContext(source('colourFieldPresentation'),v493Field);vm.runInContext(source('colourInputStyle'),v493Field);
assert(v493Field.colourInputStyle('#ffff3d').includes('color:#ffffff'));
assert(v493Field.colourInputStyle('#ffff3d').includes('--colour-text-outline:#000000'));
assert(v493Field.colourInputStyle('#ffff3d').includes('--colour-text-stroke-width:.5mm'));
assert(v493Field.colourInputStyle('#ffffff').includes('color:#111111'));
assert(v493Field.colourInputStyle('#ffffff').includes('--colour-text-outline:transparent'));
assert(v493Field.colourInputStyle('#ffffff').includes('--colour-text-stroke-width:0'));
...
assert(source('preparePdfSocaColourLayers').includes('cell.dataset.positionSummaryColours'));
assert(source('preparePdfSocaColourLayers').includes('drawPdfPositionSummaryStripeCanvas'));
assert(source('drawPdfPositionSummaryStripeCanvas').includes('drawPdfStripeCanvas(canvas,colours,scale,30,.5)'));
assert(source('positionSummaryPdfBytes').includes('patchPdfPageToJpeg(page)'));
const v494Position=vm.createContext({
  controlPositionStripStyle:()=>'--pos-pattern:test',
  positionSummaryColours:row=>[row.colour1,row.colour2,row.colour3].filter(Boolean),
  escapeAttr:value=>String(value),
  escapeHtml:value=>String(value)
});
vm.runInContext(source('positionPdfItemMarkup'),v494Position);
const v494YellowWhite=v494Position.positionPdfItemMarkup({name:'SR IMAG',colour1:'#ffff3d',colour2:'#ffffff',colour3:''});
const v494YellowLavender=v494Position.positionPdfItemMarkup({name:'SL IMAG',colour1:'#ffff3d',colour2:'#a855f7',colour3:''});
assert(v494YellowWhite.includes('data-position-summary-colours="#ffff3d,#ffffff"'));
...
vm.runInContext(source('powerMultiColourBackground'),v495Soca);
vm.runInContext(source('powerSocaMultiColourBackground'),v495Soca);
assert.equal(v495Soca.powerSocaMultiColourBackground(['#ffff3d','#ffffff']),'repeating-linear-gradient(135deg,#000 -3mm,#000 calc(-3mm + 0.3mm),#ffffff calc(-3mm + 0.3mm),#ffffff calc(4mm - 0.3mm),#000 calc(4mm - .3mm),#000 4mm,#ffff3d 4mm,#ffff3d 10mm)');
assert.equal(v495Soca.powerSocaMultiColourBackground(['#ffffff','#ffff3d']),v495Soca.powerSocaMultiColourBackground(['#ffff3d','#ffffff']));
assert(source('powerSheetRowMarkup').includes('powerSocaMultiColourBackground(socaColourValues)'));
assert(source('fanOutRowMarkup').includes('powerSocaMultiColourBackground(socaColourValues)'));
assert(source('preparePdfSocaColourLayers').includes("drawPdfSocaNameStripeCanvas,'.powerSocaNameText'"));
assert(source('drawPdfSocaNameStripeCanvas').includes('secondWidth=7*pxPerMm'));
assert(source('drawPdfSocaNameStripeCanvas').includes('firstWidth=6*pxPerMm'));
assert(source('drawPdfSocaNameStripeCanvas').includes("colour==='#ffffff'"));
...
assert(source('deviceConfigPdfRowState').includes('collapsedParents.has(parentKey)'));
...
assert(source('buildDeviceConfigPdfPages').includes('deviceConfigPdfLocationHeading'));
assert(source('buildDeviceConfigPdfPages').includes('finalisePdfPageChrome(pages)'));
assert(source('deviceConfigPdfContentFits').includes("querySelector('.pdfPageFooter')"));
assert(source('deviceConfigPdfAssetsReady').includes('document.fonts?.ready'));
...
// V50.1 fixed-width allocation uses identities for both Device Config table shapes.
const v501Keys=['status','facing','sub','name','patch','type','ip','subnet','protocol','direction','vlan','location','notes','delete'];
const v501ConsoleKeys=[...v501Keys.slice(0,-2),'role','softwareMode','softwareVersion','capacity','notes','delete'];
for(const keys of [v501Keys,v501ConsoleKeys])for(const available of [320,1410,2400]){
  const widths=c.calculateDeviceConfigWidths(keys.map(()=>90),available,false,[],keys===v501ConsoleKeys,keys);
  assert.equal(widths[0],20);assert.equal(widths.at(-1),34);
  if(available>=1410)assert(Math.abs(widths.reduce((a,b)=>a+b,0)-available)<1e-6);
  const hidden=keys.map(key=>['vlan','type'].includes(key)),hiddenWidths=c.calculateDeviceConfigWidths(keys.map(()=>90),available,false,hidden,false,keys);
  hidden.forEach((value,index)=>{if(value)assert.equal(hiddenWidths[index],0)});assert.equal(hiddenWidths[0],20);assert.equal(hiddenWidths.at(-1),34);
}
assert.equal(c.deviceConfigHeaderKey({dataset:{},classList:['deviceConfigDeleteCol']}),'delete');
assert.equal(c.deviceConfigHeaderKey({dataset:{deviceConfigColumn:'softwareMode'},classList:[]}),'softwareMode');
assert(source('measureDeviceConfigTable').includes('headers.map(deviceConfigHeaderKey)'));
for(const key of ['location','role','softwareMode','softwareVersion','capacity'])assert(dcConsole.includes(`'${key}')`),'Missing Console sort action: '+key);
const v501Devices=[{source:'console',id:'one',name:'Zulu'},{source:'console',id:'two',name:'Alpha'}],v501Items={one:{role:'backup',softwareMode:'Z mode',softwareVersion:'10.0',capacity:200},two:{role:'master',softwareMode:'A mode',softwareVersion:'2.0',capacity:100}};
const v501Sort=vm.createContext({DEVICE_CONFIG_COLUMN_KEYS:['name'],deviceConfigTableSorts:new Map(),ipAddressSourceItem:(_,id)=>v501Items[id],deviceConfigReference:()=>null,controlDeviceSoftwareVersionText:item=>item.softwareVersion,storedDeviceCapacity:item=>item.capacity,compactConsoleCapacityText:value=>String(value),render:()=>{}});
for(const name of ['deviceConfigTableSortState','setDeviceConfigTableSort','deviceConfigSortDevices','deviceConfigSortValue'])vm.runInContext(source(name),v501Sort);
for(const key of ['softwareMode','softwareVersion','capacity']){v501Sort.setDeviceConfigTableSort('room:console',key);assert.deepEqual(Array.from(v501Sort.deviceConfigSortDevices(v501Devices,'room:console'),d=>d.id),['two','one']);assert.equal(v501Sort.deviceConfigTableSortState('room:devices').key,'name')}
v501Sort.setDeviceConfigTableSort('room:console','role');assert.deepEqual(Array.from(v501Sort.deviceConfigSortDevices(v501Devices,'room:console'),d=>d.id),['one','two']);

// Shared controls route to the correct preview without modifying Power controls.
const v501Routes=[],v501Controls={deviceConfigPdfPaperSize:{},deviceConfigPdfOrientation:{},powerPdfPaperSize:{value:'A0'},powerPdfOrientation:{value:'portrait'}};
const v501Route=vm.createContext({pdfPreviewLogoState:{mode:'deviceConfig',modalId:'deviceConfigPdfModal',logos:{vendor:true}},exportLogoAvailability:()=>({vendor:true}),syncPdfPreviewLogoControls:()=>{},renderDeviceConfigPdfPages:()=>v501Routes.push('device'),renderPowerPdfPreview:()=>v501Routes.push('power'),$:id=>v501Controls[id],document:{querySelector:()=>null}});
for(const name of ['togglePdfPreviewLogo','applyProjectPdfDefaultsToPreview','updatePdfPreviewDpi'])vm.runInContext(source(name),v501Route);
v501Route.togglePdfPreviewLogo('vendor');assert.deepEqual(v501Routes,['device']);
v501Route.applyProjectPdfDefaultsToPreview('deviceConfig',{paper:'A3',orientation:'landscape',dpi:450});assert.equal(v501Controls.deviceConfigPdfPaperSize.value,'A3');assert.equal(v501Controls.deviceConfigPdfOrientation.value,'landscape');assert.equal(v501Controls.powerPdfPaperSize.value,'A0');assert.equal(v501Controls.powerPdfOrientation.value,'portrait');
v501Route.updatePdfPreviewDpi({value:'150'});assert.equal(v501Route.pdfPreviewLogoState.dpi,150);assert.deepEqual(v501Routes,['device','device']);
assert(source('preparePdfPreviewFormatting').includes("mode==='deviceConfig'?'[data-device-config-pdf-zoom]'"));assert(source('organisePdfPreviewSidePane').includes("'deviceConfigPdfInfo'"));

// Exclusion never cascades, and collapsing a Review tree cannot alter print rows.
const v501Pdf=vm.createContext({});for(const name of ['deviceConfigPdfRowState','deviceConfigPdfColumnWidths','deviceConfigPdfRenderCurrent'])vm.runInContext(source(name),v501Pdf);
const v501Session={excludedRows:new Set(['table|parent:console:a']),collapsedParents:new Set(['table|parent:console:a'])};
assert.equal(v501Pdf.deviceConfigPdfRowState('table|parent:console:a','',v501Session,false).render,false);
assert.equal(v501Pdf.deviceConfigPdfRowState('table|port:console:a:1','table|parent:console:a',v501Session,false).render,true);
assert.equal(v501Pdf.deviceConfigPdfRowState('table|port:console:a:1','table|parent:console:a',v501Session,true).render,false);
assert.equal(v501Pdf.deviceConfigPdfRowState('table|parent:console:a','',v501Session,true).dimmed,true);
v501Session.collapsedParents.clear();assert.equal(v501Pdf.deviceConfigPdfRowState('table|port:console:a:1','table|parent:console:a',v501Session,true).render,true);
for(const width of [700,1000,1500]){const widths=v501Pdf.deviceConfigPdfColumnWidths(v501ConsoleKeys.slice(0,-1),[],width);assert.equal(widths[0],38);assert(Math.abs(widths.reduce((a,b)=>a+b,0)-width)<1e-6)}
assert(source('preparePdfSocaColourLayers').includes(".deviceConfigPdfTable .controlLocationCell[data-power-position-colours]"));
assert(html.includes('.deviceConfigPdfTable .controlLocationCell,.deviceConfigPdfTable .controlLocationCell .deviceConfigPdfValue'));
assert(html.includes('font-size:14px!important;font-weight:900!important;height:auto!important'));
assert(source('deviceConfigPdfDisplayOnly').includes('controlLocationCellStyle(cell.textContent.trim())'));

// Small DOM test double: execute conversion and row preparation without a browser.
class V501Element{
  constructor(tag='div'){this.tagName=tag.toUpperCase();this.children=[];this.parentElement=null;this.attrs={};this.dataset={};this.style={setProperty:(k,v)=>{this.style[k]=v}};this.hidden=false;this.value='';this.text='';this.clientWidth=900;this.classList={contains:k=>this.className.split(' ').includes(k),add:(...keys)=>{this.className=[...new Set([...this.className.split(' ').filter(Boolean),...keys])].join(' ')},remove:k=>{this.className=this.className.split(' ').filter(x=>x!==k).join(' ')},toggle:(k,on)=>{if(on===undefined)on=!this.classList.contains(k);on?this.classList.add(k):this.classList.remove(k);return on}}}
  ...
  setAttribute(key,value){this.attrs[key]=String(value);if(key.startsWith('data-'))this.dataset[key.slice(5).replace(/-([a-z])/g,(_,v)=>v.toUpperCase())]=String(value)}
  getAttribute(key){return this.attrs[key]??null}removeAttribute(key){delete this.attrs[key];if(key.startsWith('data-'))delete this.dataset[key.slice(5).replace(/-([a-z])/g,(_,v)=>v.toUpperCase())]}
  appendChild(child){child.remove();child.parentElement=this;this.children.push(child);return child}append(...children){children.forEach(child=>this.appendChild(child))}prepend(child){child.remove();child.parentElement=this;this.children.unshift(child)}
  remove(){if(this.parentElement){this.parentElement.children=this.parentElement.children.filter(child=>child!==this);this.parentElement=null}}
  replaceChildren(...children){this.children.forEach(child=>child.parentElement=null);this.children=[];this.append(...children)}replaceWith(other){const parent=this.parentElement,index=parent.children.indexOf(this);parent.children[index]=other;other.parentElement=parent;this.parentElement=null}
  matches(selector){selector=selector.trim();const not=selector.match(/:not\(([^)]+)\)/);if(not){if(this.matches(not[1]))return false;selector=selector.replace(not[0],'')}const attr=selector.match(/\[([^=\]]+)(?:="([^"]*)")?\]/);if(attr){const value=attr[1].startsWith('data-')?this.dataset[attr[1].slice(5).replace(/-([a-z])/g,(_,v)=>v.toUpperCase())]:this.getAttribute(attr[1]);if(value===undefined||value===null||(attr[2]!==undefined&&value!==attr[2]))return false;selector=selector.replace(attr[0],'')}const classes=selector.match(/\.[\w-]+/g)||[];if(classes.some(cls=>!this.classList.contains(cls.slice(1))))return false;const tag=selector.replace(/\.[\w-]+/g,'');return !tag||tag==='*'||this.tagName===tag.toUpperCase()}
  querySelectorAll(selector){const all=this.children.flatMap(child=>[child,...child.querySelectorAll('*')]);return all.filter(child=>selector.split(',').some(part=>{const pieces=part.trim().split(/\s+/);if(!child.matches(pieces.pop()))return false;let parent=child.parentElement;while(pieces.length){const wanted=pieces.pop();while(parent&&!parent.matches(wanted))parent=parent.parentElement;if(!parent)return false;parent=parent.parentElement}return true}))}
  querySelector(selector){return this.querySelectorAll(selector)[0]||null}
  get cells(){return this.children.filter(child=>['TD','TH'].includes(child.tagName))}get rows(){return this.querySelectorAll('tr')}get tBodies(){return this.children.filter(child=>child.tagName==='TBODY')}get tHead(){return this.children.find(child=>child.tagName==='THEAD')||null}
  cloneNode(deep=false){const copy=new V501Element(this.tagName);copy.attrs={...this.attrs};copy.dataset={...this.dataset};copy.hidden=this.hidden;copy.value=this.value;copy.type=this.type;copy.text=this.text;copy.selectedOptions=this.selectedOptions;copy.testHeight=this.testHeight;if(deep)this.children.forEach(child=>copy.appendChild(child.cloneNode(true)));return copy}
  insertAdjacentHTML(_,text){const node=new V501Element('div');node.textContent=text;this.append(node)}
}
const v501Doc={createElement:tag=>new V501Element(tag),body:new V501Element('body')},v501Dom=vm.createContext({document:v501Doc,positionForName:()=>null,colourTextToHex:v=>v,controlLocationCellStyle:()=>'',safeEncodeURIComponent:encodeURIComponent});
for(const name of ['deviceConfigPdfRowState','deviceConfigPdfDisplayOnly','deviceConfigPdfPrepareTable'])vm.runInContext(source(name),v501Dom);
const v501Table=new V501Element('table');v501Table.dataset.deviceConfigTableKey='room:devices';const v501Head=new V501Element('thead'),v501HeadRow=new V501Element('tr');v501Head.append(v501HeadRow);v501HeadRow.append(new V501Element('th'));v501Table.append(v501Head);const v501Body=new V501Element('tbody');v501Table.append(v501Body);
function v501Row(id,parent='',tree=false){const row=new V501Element('tr');row.dataset.deviceConfigRowId=id;if(parent)row.dataset.deviceConfigParent=parent;const status=new V501Element('td');status.className='deviceConfigStatusCol';row.append(status);if(tree){const button=new V501Element('button');button.className='deviceConfigTreeButton';button.setAttribute('onclick','mutateProject()');status.append(button)}const cell=new V501Element('td'),input=new V501Element('input');input.value='Saved value';input.setAttribute('onchange','mutateProject()');input.setAttribute('data-dc-id','device');input.setAttribute('contenteditable','true');cell.append(input);row.append(cell);const del=new V501Element('td');del.className='deviceConfigDeleteCol';row.append(del);return row}
v501Body.append(v501Row('parent:network:a','',true),v501Row('port:network:a:p1','network:a'),v501Row('interface:network:a:2','network:a'));
const v501TreeSession={excludedRows:new Set(['room:devices|parent:network:a']),collapsedParents:new Set(['room:devices|parent:network:a'])};
const v501Review=v501Dom.deviceConfigPdfPrepareTable(v501Table,true,v501TreeSession),v501Print=v501Dom.deviceConfigPdfPrepareTable(v501Table,false,v501TreeSession);
assert.equal(v501Review.tBodies[0].rows.length,1);assert(v501Review.tBodies[0].rows[0].classList.contains('deviceConfigPdfExcluded'));assert.equal(v501Review.querySelectorAll('.deviceConfigPdfEye').length,1);
assert.equal(v501Print.tBodies[0].rows.length,2);assert.equal(v501Print.querySelectorAll('input,select,textarea,button').length,0);assert.equal(v501Print.querySelectorAll('[data-dc-id]').length,0);assert.equal(v501Print.querySelectorAll('[contenteditable]').length,0);assert.equal(v501Print.querySelectorAll('[onchange]').length,0);assert.equal(v501Print.querySelector('.deviceConfigPdfValue').textContent,'Saved value');assert.equal(v501Table.querySelectorAll('input').length,3,'Source project table remains editable');
v501TreeSession.collapsedParents.clear();const v501Expanded=v501Dom.deviceConfigPdfPrepareTable(v501Table,true,v501TreeSession);assert.equal(v501Expanded.querySelectorAll('.deviceConfigPdfEye').length,3);
// Selects and segmented addresses retain displayed values without any input handlers.
const v501Values=new V501Element('table'),v501Select=new V501Element('select');v501Select.selectedOptions=[{textContent:'VLAN 10 — Control'}];v501Values.append(v501Select);const v501Address=new V501Element('div');v501Address.className='ipSegmentedField';['10','0','0','42'].forEach((value,index)=>{const input=new V501Element('input');input.value=value;input.setAttribute('data-ip-octet',index);v501Address.append(input)});v501Values.append(v501Address);v501Dom.deviceConfigPdfDisplayOnly(v501Values);assert.deepEqual(v501Values.querySelectorAll('.deviceConfigPdfValue').map(node=>node.textContent),['VLAN 10 — Control','10.0.0.42']);
console.log('PASS: V50.1 identity widths, complete table sorting, PDF controls, inclusion and display-only rows.');

// Execute the paginator with deterministic measured row heights (no browser layout).
let v501Finalised=[],v501Oversized=0;
const v501PageHeight=page=>40+page.querySelectorAll('.deviceConfigPdfLocationHeading').length*10+page.querySelectorAll('.deviceConfigPdfTableWrap').length*10+page.querySelectorAll('tr[data-device-config-pdf-row-key]').reduce((sum,row)=>sum+(row.testHeight||20),0);
const v501Build=vm.createContext({document:v501Doc,deviceConfigPdfPrepareTable:v501Dom.deviceConfigPdfPrepareTable,deviceConfigPdfCreatePage:(host,d,index)=>{const page=new V501Element('div');page.className='exportPage';page.capacity=index?120:100;const documentHost=new V501Element('div');documentHost.className='deviceConfigPdfDocument';const grid=new V501Element('table');grid.className='deviceConfigPdfVlanGrid';documentHost.append(grid);page.append(documentHost);host.append(page);return {page,documentHost}},deviceConfigPdfReserveChrome:()=>{},deviceConfigPdfTableColumns:()=>{},deviceConfigPdfContentFits:page=>v501PageHeight(page)<=page.capacity,deviceConfigPdfFitOversizedPage:state=>{state.full=true;v501Oversized++},finalisePdfPageChrome:pages=>{v501Finalised=pages;pages.forEach((page,index)=>{page.number=index+1;page.total=pages.length})}});
vm.runInContext(source('buildDeviceConfigPdfPages'),v501Build);
const v501Many=v501Table.cloneNode(true);v501Many.tBodies[0].replaceChildren(...Array.from({length:8},(_,i)=>v501Row(i===0?'parent:network:a':`port:network:a:${i}`,i?'network:a':'',!i)));
const v501ManyGroups=[{key:'room',location:'Front',tables:[v501Many]}],v501All={hiddenLocations:new Set(),excludedRows:new Set(),collapsedParents:new Set()},v501Host=new V501Element('div');
let v501Pages=v501Build.buildDeviceConfigPdfPages(v501Host,v501ManyGroups,{},v501All,false);assert.equal(v501Pages.length,3);assert.deepEqual(Array.from(v501Pages,page=>page.querySelectorAll('tr[data-device-config-pdf-row-key]').length),[2,3,3]);assert(v501Pages.every(page=>v501PageHeight(page)<=page.capacity));assert(v501Pages.every(page=>page.querySelector('.deviceConfigPdfVlanGrid')&&page.querySelector('thead')&&page.querySelector('.deviceConfigPdfLocationHeading')));assert(v501Pages.every(page=>page.total===3));assert(v501Pages[1].querySelector('.deviceConfigPdfLocationHeading').textContent.includes('continued'));assert.equal(v501Oversized,0);
v501All.excludedRows.add('room:devices|parent:network:a');v501All.collapsedParents.add('room:devices|parent:network:a');v501Pages=v501Build.buildDeviceConfigPdfPages(new V501Element('div'),v501ManyGroups,{},v501All,false);assert.equal(v501Pages.flatMap(page=>page.querySelectorAll('tr[data-device-config-pdf-row-key]')).length,7);assert(!v501Pages.some(page=>page.querySelector('[data-device-config-pdf-row-key="room:devices|parent:network:a"]')));
const v501ReviewPages=v501Build.buildDeviceConfigPdfPages(new V501Element('div'),v501ManyGroups,{},v501All,true);assert.equal(v501ReviewPages.length,1);assert.equal(v501ReviewPages[0].querySelectorAll('.deviceConfigPdfEye').length,1);
const v501Short=v501Table.cloneNode(true);v501Short.tBodies[0].replaceChildren(v501Row('parent:network:a'),v501Row('port:network:a:1','network:a'));const v501Next=v501Table.cloneNode(true);v501Next.tBodies[0].replaceChildren(v501Row('parent:network:b'));const v501Separate=v501Build.buildDeviceConfigPdfPages(new V501Element('div'),[{key:'one',location:'Front',tables:[v501Short]},{key:'two',location:'Rear',tables:[v501Next]}],{},{hiddenLocations:new Set(),excludedRows:new Set(),collapsedParents:new Set()},false);assert.equal(v501Separate.length,2);assert.equal(v501Separate[1].querySelector('.deviceConfigPdfLocationHeading').textContent,'Rear','A new table moved to the next page is not a continuation');
assert(source('deviceConfigPdfPrepareTable').includes('<svg width="12" height="12"'),'Review uses a visible eye icon');
v501All.hiddenLocations.add('room');const v501Empty=v501Build.buildDeviceConfigPdfPages(new V501Element('div'),v501ManyGroups,{},v501All,false);assert.equal(v501Empty.length,1);assert.equal(v501Empty[0].querySelectorAll('tr[data-device-config-pdf-row-key]').length,0);
console.log('PASS: V50.1 repeated VLAN/continuation headings, footer-safe pagination and non-cascading PDF inclusion.');

(async()=>{
  // Pagination waits until both fonts and images settle.
  let finishFonts,finishImage,ready=false;const readyContext=vm.createContext({document:{fonts:{ready:new Promise(resolve=>finishFonts=resolve)}},requestAnimationFrame:fn=>fn()});vm.runInContext(source('deviceConfigPdfAssetsReady'),readyContext);
  const wait=readyContext.deviceConfigPdfAssetsReady({offsetHeight:100,querySelectorAll:()=>[{decode:()=>new Promise(resolve=>finishImage=resolve)}]}).then(()=>ready=true);await Promise.resolve();assert.equal(ready,false);finishFonts();await Promise.resolve();assert.equal(ready,false);finishImage();await wait;assert.equal(ready,true);
  // A late, superseded render cannot replace pages created by a newer request.
  const waits=[],committed=[],raceSession={renderVersion:0,hiddenLocations:new Set(),excludedRows:new Set(),collapsedParents:new Set()};
  const race=vm.createContext({document:v501Doc,console,deviceConfigPdfSession:raceSession,deviceConfigPdfSourceGroups:()=>[],deviceConfigPdfDimensions:()=>({paper:'A4'}),deviceConfigPdfCreatePage:(stage)=>{const documentHost=new V501Element('div');stage.append(documentHost);return {documentHost}},deviceConfigPdfAssetsReady:()=>new Promise(resolve=>waits.push(resolve)),deviceConfigPdfLocationControls:()=>{},buildDeviceConfigPdfPages:host=>{const page=new V501Element('div');page.className='exportPage';host.append(page);return [page]},showDeviceConfigPdfView:()=>committed.push(raceSession.renderVersion),$:()=>null});
  for(const name of ['deviceConfigPdfRenderCurrent','renderDeviceConfigPdfPages'])vm.runInContext(source(name),race);
  const first=race.renderDeviceConfigPdfPages(),second=race.renderDeviceConfigPdfPages();waits[1]();await second;const accepted=raceSession.printHost;waits[0]();await first;assert.deepEqual(committed,[2]);assert.equal(raceSession.printHost,accepted);assert.equal(v501Doc.body.children.length,0,'Staging nodes are removed');
  // Download captures the exact existing Print Preview pages and restores Review.
  const printHost=new V501Element('div'),captured=[];const pageA=new V501Element('div'),pageB=new V501Element('div');pageA.className=pageB.className='exportPage';printHost.append(pageA,pageB);const exportSession={view:'review',pending:Promise.resolve(),printHost,dimensions:{ptW:800,ptH:600}},modal={inert:false},viewChanges=[];
  const capture=vm.createContext({deviceConfigPdfSession:exportSession,$:()=>modal,requestAnimationFrame:fn=>fn(),showDeviceConfigPdfView:()=>viewChanges.push(exportSession.view),patchPdfPageToJpeg:async page=>{captured.push(page);return page===pageA?'A':'B'},buildPdfImageFile:(images,w,h)=>({images,w,h})});vm.runInContext(source('deviceConfigPreviewPdfBytes'),capture);
  const result=await capture.deviceConfigPreviewPdfBytes();assert.deepEqual(captured,[pageA,pageB]);assert.deepEqual(Array.from(result.images),['A','B']);assert.equal(result.w,800);assert.equal(result.h,600);assert.deepEqual(viewChanges,['print','review']);assert.equal(modal.inert,false);assert.equal(exportSession.exporting,false);
  console.log('PASS: V50.1 font/image readiness, stale-render cancellation and identical Print Preview/download pages.');
})().catch(error=>{console.error(error);process.exitCode=1});


require('./patch-address-review.cjs');

require('./gdtf-parameters.cjs');

require('./patch-summary-merged.cjs');

require('./patch-summary-notes.cjs');
