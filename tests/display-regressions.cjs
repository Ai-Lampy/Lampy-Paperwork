...
const patchNavCells=[];
for(let row=0;row<2;row++)for(let col=0;col<2;col++){const key=`${row}:${col}`,control={dataset:{patchGridRow:String(row),patchGridCol:String(col)},focus:()=>{patchFocused=key},select:()=>{patchSelected=key}};patchNavCells.push({key,querySelector:()=>control})}
