...
for(const file of ['info_txt/welcome_message.json','info_txt/walkthrough.json']){const doc=JSON.parse(fs.readFileSync(root+file));assert(doc.title.includes('V43'));assert(JSON.stringify(doc).includes('Fixture'));assert(JSON.stringify(doc).length>300)}
...
for(const name of ['normaliseImportMatch','fixtureOverrideKey','normaliseFixtureOverrides','normaliseLabelPreview','positionGridNumber','normalisePositionSummaryFormat','defaultProjectInfo','normaliseProjectInfo'])vm.runInContext(source(name),pctx);
...
assert(fs.readFileSync(root+'tests/BROWSER-CHECKLIST.md','utf8').startsWith('# V43 browser release checks'));
...
assert.equal(appVersion,'43.1');
assert(html.includes('style="width:min(1300px,100%);"'));
...
assert(source('positionSummaryPdfBytes').includes('geometry.topMm'));
...
assert(source('drawPositionSummaryStripeFill').includes('.5*PDF_MM_TO_PT'));
